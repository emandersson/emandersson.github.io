...
<p><b>GPS:</b> The app won't settle with anything but the best (GPS if available). Note! resolving to GPS requires relatively much power.</p>`;
  var imgH=imgHelp.cloneNode(1).css({'margin-right':'auto'});   popupHover(imgH, createElement('div').myHtml(strHelp), 30);
  //var aInfo=createElement('a').prop({href:'https://info.closeby.market/universalTracker_settings'}).myText('more info').css({'margin-left':'0.4em', 'margin-right':'auto'});
...
//var uWiki='https://closeby.market/closebymarketcontrol';
var uWiki='https://gavott.com/selfTracker';
var uListOfValidSites='https://gavott.com/selfTracker_ListOfValidSites';
...
var getItems=function(){
  var str, obj; if(boNativeAndroid) str=Android.readPref(); else str=getItem('selfTracker');
...
var setItems=function(){
  ...
  //if(boNativeAndroid) Android.writePref(str); else setItem('selfTracker', str);
  if(boNativeAndroid) ; else setItem('selfTracker', str);
}
